...
import { 
  ...
  Loader2, FileVideo, Eye, LayoutGrid, Terminal, ArrowLeft, Database
} from 'lucide-react';
...
function VideoAnalyzer({ onBack }) {
  ...
  const handleFile = (selectedFile) => {
    if (selectedFile?.type?.startsWith('video/')) {
      ...
      setError("Upload error: Unsupported file format");
    }
  };
  ...
  const analyzeVideo = async () => {
    ...
    try {
      ...
      setError("System offline: Analyzer disconnected");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen text-white selection:bg-purple-500/30 pb-32 relative bg-black">
            ...
            {!file ? (
              <div className="space-y-12">
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className={`glass rounded-2xl p-16 flex flex-col items-center border-[1px] transition-all duration-300 cursor-pointer ${
                    dragActive ? "border-purple-400 bg-purple-900/10 shadow-[0_0_20px_rgba(168,85,247,0.1)]" : "border-white/20 hover:border-purple-500/50"
                  }`}
                ...
                >
                  ...
                  <h2 className="text-2xl font-bold mb-6 text-center leading-relaxed">
                    Upload video document <br/>
                    <span className="text-white/60">for testing anomalies</span>
                  </h2>
                  ...
                  <input ref={fileInputRef} type="file" className="hidden" accept="video/*" onChange={(e) => handleFile(e.target.files[0])} />
                </motion.div>
                ...
                >
                  <div className="flex flex-col gap-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-xl tracking-tight text-white truncate max-w-[180px]">{file.name}</h3>
                            ...
                            <div className="relative z-20 flex flex-col items-center gap-4 text-center px-4">
                               ...
                               <div className="space-y-1">
                                  <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-white italic animate-pulse">
                                     {(() => {
                                        const now = Date.now();
                                        const step = Math.floor((now / 1500) % 5);
                                        const steps = [
                                          "Initializing Inference Engine...",
                                          "Extracting Spatial Features (CNN)...",
                                          "Analyzing Temporal Dependencies (BiLSTM)...",
                                          "Computing Attention Weights...",
                                          "Finalizing XGBoost Classification..."
                                        ];
                                        return steps[step];
                                     })()}
                                  </p>
                                  <p className="text-[8px] text-purple-400 font-mono opacity-50 uppercase tracking-widest">
                                     Live Neural Scan in Progress
                                  </p>
                               </div>
                            </div>
                         </div>
                        ...
                        <div className={`glass rounded-2xl p-8 border-l-6 ${
                          ...
                          <div className="flex flex-col gap-8">
                            ...
                            <div className="grid grid-cols-2 gap-6 border-y border-white/10 py-8">
                               <div>
                                  <p className="text-[9px] uppercase font-bold text-white/40 mb-1">Mean Score</p>
                                  <p className="text-2xl font-bold tracking-tight text-white">{(result.average_score * 100).toFixed(1)} %</p>
                               </div>
                               <div>
                                  <p className="text-[9px] uppercase font-bold text-white/40 mb-1">Time</p>
      ...
      <footer className="mt-24 w-full p-12 flex flex-col items-center gap-2 border-t border-white/10 text-white font-bold uppercase tracking-[0.2em] text-[9px]">
        <span className="opacity-40">Industrial Process Anomaly Detection © 2026</span>
